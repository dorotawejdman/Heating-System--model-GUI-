...
f.truncate()
Tr=[flaga,0,0]
with open("E:\Air\PKSS\GUI_PKSS\Python_projekty\TrSP.txt", 'a', encoding='utf-8', newline='') as csvfile:
...
with open('Wyniki.csv', mode='r') as csv_file:
    ...
    for row in csv_reader:
        if line_count == 0:
            ...
            Tr2.place(x=687,y=440)
            #TrSP
            TrSP2 = Label(window, text=str(row[14])[0:4]+" [°C]", fg="black",bg="orange", font="none 12")
            TrSP2.place(x=777,y=440)
            #Tpco
            Tpco1 = Label(window, text=str(row[2])[0:4]+" [°C]", fg="black",bg="orange", font="none 12")
            Tpco1.place(x=867,y=440)
            #Fco1
            fco1 = str("{0:.2f}".format(float(row[11][0:6]) * 40))
            Fco1 = Label(window, text=str(fzm)+" [t/h]", fg="black",bg="green", font="none 12")
            Fco1.place(x=957,y=440)
