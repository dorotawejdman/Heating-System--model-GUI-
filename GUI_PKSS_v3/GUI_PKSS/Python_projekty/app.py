...
name='wyniki2.csv' #xlsx name in the same folder as project
...
nastawy = [P_Um, I_Um, D_Um, P_Ub1, I_Ub1, D_Ub1, P_Ub2, I_Ub2, D_Ub2]

flaga = 0
f = open("TrSP.txt", "w")
f.truncate()
Tr = [flaga, 0, 0]
with open("E:\Air\PKSS\GUI_PKSS_v3\GUI_PKSS\Python_projekty\TrSP.txt", 'a', encoding='utf-8', newline='') as csvfile:
    csvwriter = csv.writer(csvfile, delimiter=";")
    for i in range(0, 3):
        csvwriter.writerow([Tr[i]])

f = open("PID.txt", "w")
f.truncate()
with open("E:\Air\PKSS\GUI_PKSS_v3\GUI_PKSS\Python_projekty\PID.txt", 'a', encoding='utf-8', newline='') as csvfile:
    csvwriter = csv.writer(csvfile, delimiter=";")
    for i in range(0, 9):
        csvwriter.writerow([nastawy[i]])
...
def writePID(P,I,D,d):
    ...
    f.truncate()
    nastawy[d*3]=float(P)
    nastawy[d*3+1]=float(I)
    nastawy[d*3+2]=float(D)
    os.chdir('E:\\Air\\PKSS\\GUI_PKSS_v3\\GUI_PKSS\\Python_projekty\\')
    with open("E:\Air\PKSS\GUI_PKSS_v3\GUI_PKSS\Python_projekty\PID.txt", 'a', encoding='utf-8', newline='') as csvfile:
            csvwriter=csv.writer(csvfile,delimiter=";")
            for i in range(0,9):
                csvwriter.writerow([nastawy[i]])
...
def Tr_ok():
    ...
    Tr1 = Trent1.get()
    Tr2 = Trent2.get()
    flaga=1 #flaga ustawiana na 1 jesli uzytkownik poda wartosci zadane
    Tr=[flaga,float(Tr1),float(Tr2)]
    with open("E:\Air\PKSS\GUI_PKSS_v3\GUI_PKSS\Python_projekty\TrSP.txt", 'a', encoding='utf-8', newline='') as csvfile:
            csvwriter=csv.writer(csvfile,delimiter=";")
            for i in range (0,3):
                csvwriter.writerow([Tr[i]])
...
def uruchom_program():
    i = 0
    os.startfile('"E:\Air\PKSS\GUI_PKSS_v3\GUI_PKSS\Python_projekty\pkss_projekt.exe"')
    try:
        while (i < 9999):
            i += 1
            with open(name, mode='r') as csv_file:
                ...
                for row in csv_reader:
                    ...
                    # Budynek 2
                    # Tr
                    Tr2 = Label(window, text=str(row[13])[0:4] + " [°C]", fg="black", bg="orange", font="none 12")
                    Tr2.place(x=687, y=450)
                    # TrSP
                    TrSP2 = Label(window, text=str(row[14])[0:4] + " [°C]", fg="black", bg="orange", font="none 12")
                    TrSP2.place(x=777, y=450)
                    # Tpco
                    Tpco1 = Label(window, text=str(row[2])[0:4] + " [°C]", fg="black", bg="orange", font="none 12")
        ...
        print("Unexpected Error but have no idea which kind - sorry ")
